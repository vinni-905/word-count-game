...
# MODIFIED: Import from database module
try:
    from database import get_random_approved_user_puzzle
except ImportError:
    # This basicConfig is a fallback if the main one in main.py hasn't run yet (e.g. direct script execution)
    logging.basicConfig(level=logging.ERROR, format='%(asctime)s - %(levelname)s - [%(module)s:%(lineno)d] - %(message)s')
    logging.error("CRITICAL IMPORT ERROR: Could not import 'get_random_approved_user_puzzle' from database module. User-submitted puzzles will be disabled.")
    def get_random_approved_user_puzzle() -> Optional[Dict[str, Any]]: # Fallback stub
        return None

# --- Initialize Logger ---
logger = logging.getLogger(__name__)
if not logger.hasHandlers(): # Check if handlers are already set (e.g., by main.py)
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - [%(module)s:%(lineno)d] - %(message)s')

# --- Constants ---
MODEL_DIR = "model"
BEST_MODEL_NAME_FROM_TRAINING = "ridge_regression" # Should be set to your actual model base name
...
WORD2VEC_ACTUAL_FILENAME = "glove.6B.100d.kv" # Should be your actual W2V filename
...
# ... (Keep your existing ML model loading logic here - it seemed to be working)
try:
...
# --- Load Word2Vec Model ---
w2v_model = None
# ... (Keep your existing Word2Vec loading logic here - it seemed to be working)
if GENSIM_AVAILABLE:
    try:
...
# --- Load Enriched Vocabulary & Categories ---
...
word_categories_hardcoded = { # Fallback
    ...
    'kitchen_items': ['knife', 'fork', 'spoon', 'plate', 'bowl', 'cup', 'pan', 'pot']
}
word_categories: Dict[str, List[str]] = {}
# ... (Keep your existing vocabulary loading logic here - it seemed to be working and populating word_categories)
...
connection_types = {
    ...
    'word_length': 3,
    'user_submitted': 5 # Default complexity for user puzzles
}
connection_descriptions = {
    ...
    'word_length': "Same Word Length",
    'user_submitted': "Community Puzzle"
}
...
def _generate_groups_by_category(num_groups_to_gen: int, words_per_group_val: int, used_words_global: Set[str], generation_params: Dict[str, Any]) -> Optional[Tuple[Dict[str, List[str]], List[str], Dict[str, str], Dict[str, int]]]:
    ...
    if len(all_cat_names) < num_groups_to_gen:
        logger.warning(f"CatGen: Not enough distinct categories ({len(all_cat_names)}) for {num_groups_to_gen} groups. Using replacement.")
    ...
    for i, cat_name in enumerate(selected_cat_names):
        ...
        if len(group_words) < words_per_group_val:
            logger.warning(f"CatGen: Failed for cat '{cat_name}', rarity '{target_word_rarity}'. Found {len(group_words)}.")
            return None
...
def generate_solvable_puzzle(target_difficulty: str, user_performance_summary: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # --- MODIFIED: Integrate User-Submitted Puzzles ---
    USE_USER_PUZZLE_CHANCE = 0.25 # 25% chance to try a user puzzle
    if get_random_approved_user_puzzle is not None and random.random() < USE_USER_PUZZLE_CHANCE: # Check if function exists
        logger.info(f"Attempting to select an approved user-submitted puzzle (Chance: {USE_USER_PUZZLE_CHANCE*100}%).")
        user_puzzle_data_from_db = get_random_approved_user_puzzle()
        if user_puzzle_data_from_db:
            ...
            solution_groups_temp = {}
            all_words_list_temp = []
            group_descriptions_temp = {}
            temp_difficulty_index_map = {}
            valid_user_puzzle = True

            for i in range(1, 5):
                group_key = f"group_{i}"
                words_for_group = user_puzzle_data_from_db.get(f'group{i}_words', [])
                ...
                if len(processed_words) != WORDS_PER_GROUP:
                    logger.error(f"User puzzle ID {user_puzzle_data_from_db['id']} group {i} does not have {WORDS_PER_GROUP} valid unique words. Words: {words_for_group}, Processed: {processed_words}. Skipping.")
                    valid_user_puzzle = False; break
                
                solution_groups_temp[group_key] = processed_words
                all_words_list_temp.extend(processed_words)
                group_descriptions_temp[group_key] = user_puzzle_data_from_db.get(f'group{i}_description', f"Community Group {i}")
                temp_difficulty_index_map[group_key] = i - 1 # Simple 0-3 index

            if valid_user_puzzle and len(set(all_words_list_temp)) != num_words_total:
                logger.error(f"User puzzle ID {user_puzzle_data_from_db['id']} does not contain {num_words_total} unique words across all groups. Found {len(set(all_words_list_temp))}. Skipping.")
                valid_user_puzzle = False
            
            if valid_user_puzzle:
                ...
                if 'user_submitted' not in connection_types: connection_types['user_submitted'] = 5
                
                final_puzzle_difficulty_data = predict_difficulty_for_params(user_puzzle_gen_params, actual_selected_words=all_words_list_temp)
                ...
                random.shuffle(all_words_list_temp)
                active_puzzles[puzzle_id] = {
                    "puzzle_id": puzzle_id,
                    "words_on_grid": all_words_list_temp,
                    "solution": solution_groups_temp,
                    "descriptions": group_descriptions_temp,
                ...
                logger.info("Selected user puzzle was invalid, proceeding with algorithmic generation.")
        elif get_random_approved_user_puzzle is not None and USE_USER_PUZZLE_CHANCE > 0 :
             logger.info("Did not select a user puzzle this time / No approved user puzzle found, proceeding with algorithmic generation.")
    # --- END USER-SUBMITTED PUZZLE LOGIC ---
    ...
    for attempt in range(min(MAX_GENERATION_ATTEMPTS, len(param_candidates))):
        ...
        if current_prediction.get('difficulty') == target_difficulty and not current_prediction.get('is_fallback', True):
            ...
            logger.info(f"--> Algo: Target '{target_difficulty}' MET by ML on attempt {attempt+1}. Params: {generation_params}")
            break

    if not found_matching_params:
        logger.warning(f"Algo: No exact ML match for '{target_difficulty}'. Using best found: {best_params_so_far} (Pred: {best_prediction_so_far})")
        if best_params_so_far and best_prediction_so_far:
            ...
            logger.error("Algo: No valid ML predictions or candidates. Using hardcoded fallback.")
    ...
    logger.info(f"--- Algorithmic: Finalizing words. Connection: '{final_connection_type}', TargetRarity: '{generation_params.get('word_rarity')}' ---")
    ...
    if final_connection_type == 'same_category':
        ...
        logger.warning(f"Algorithmic: Word selection logic for '{final_connection_type}' is a STUB or not implemented.")

    if not word_selection_data:
        logger.info(f"Algorithmic: Primary word selection failed for '{final_connection_type}'. Using fallback groups.")
    ...
    if not all_words_for_grid or len(all_words_for_grid) != num_words_total or len(solution_groups) != NUM_GROUPS:
        logger.critical(f"CRITICAL (Algo): Puzzle assembly error. Words: {len(all_words_for_grid if all_words_for_grid else [])}/{num_words_total}, Groups: {len(solution_groups)}/{NUM_GROUPS}. Emergency Fallback.")
        # ... (emergency fallback logic as before)
        try:
            ...
            raise ValueError("Malformed puzzle generation after multiple fallbacks.") from e_fall


    final_puzzle_difficulty_data = predict_difficulty_for_params(generation_params, actual_selected_words=all_words_for_grid)
    puzzle_id_algo = f"algo_{puzzle_id_base[:8]}"

    random.shuffle(all_words_for_grid)
    active_puzzles[puzzle_id_algo] = {
        "puzzle_id": puzzle_id_algo,
        "words_on_grid": [w.upper() for w in all_words_for_grid],
    ...
    logger.info(f"--- Algorithmic Puzzle {puzzle_id_algo} (Target: {target_difficulty}, Conn: {final_connection_type}, Final Gen Diff: {active_puzzles[puzzle_id_algo]['difficulty']}) created. ---")
    return {"puzzle_id": puzzle_id_algo, "words": active_puzzles[puzzle_id_algo]["words_on_grid"], "difficulty": active_puzzles[puzzle_id_algo]["difficulty"]}


def get_or_generate_daily_challenge() -> dict:
    # ... (This function should remain the same as your last version) ...
    cleanup_old_puzzles()
...
def cleanup_old_puzzles(): # ... (same as before) ...
...
if __name__ == "__main__": # ... (same as before, with potential additions for testing user puzzles) ...
    ...
    # Test regular puzzle generation
    for diff_test in ["easy", "medium", "hard"]:
        try:
            print(f"\n--- Testing Regular Puzzle Generation ({diff_test.capitalize()}) ---")
            perf_summary_for_test = None
            if diff_test == "medium": perf_summary_for_test = mock_good_perf_medium
            elif diff_test == "hard": perf_summary_for_test = mock_struggling_perf_hard
    ...
    try:
        ...
        # ... (rest of your daily challenge test)
    except Exception as e: # ...
        pass
